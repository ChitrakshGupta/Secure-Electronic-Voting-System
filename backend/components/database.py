...
def add_user_to_db(username, password_hash, role, device_id=None):
    """
    ...
    try:
        cursor.execute("""
            ...
            VALUES (?, ?, ?, ?)
        """, (username, password_hash, role, device_id))
        conn.commit()
...
def add_admin_user():
    ...
    try:
        cursor.execute("""
            INSERT INTO Users (username, password_hash, role, device_id)
            VALUES (?, ?, ?, ?)
        """, (admin_username, password_hash, admin_role, admin_device_id))
        conn.commit()
